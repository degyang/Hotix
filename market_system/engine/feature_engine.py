...
def _rolling_percentile(series: pd.Series, window: int) -> pd.Series:
    return series.rolling(window).apply(lambda values: float((values <= values[-1]).mean()), raw=True)


def compute_index_features(index_id: str, date: str, df: pd.DataFrame, features_dsl: dict) -> IndexRuntime:
    df_slice = df[df["date"] <= date].copy()
    last_row = df_slice.iloc[-1].to_dict()
    runtime = IndexRuntime(id=index_id, date=date, raw=last_row, features={}, states={}, trace={"features": {}})
