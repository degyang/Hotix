...
def score_market_context(market, indices: dict, pairs: dict, contexts_dsl: dict) -> dict:
    ...
    for context_def in contexts_dsl["contexts"]:
        ...
        for rule in context_def["rules"]:
            if evaluate_expression(rule["when"], resolver):
                ...
                matched_rules.append({"rule_id": rule["id"], "score": score, "evidence": rule["evidence"]})
        scores.append((context_def, total_score, evidence))
        trace[context_def["id"]] = {"label": context_def["label"], "score": total_score, "matched_rules": matched_rules}
