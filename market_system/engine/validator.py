from market_system.engine.expression import normalize_expression
...
def validate_all_dsl(dsl: dict, registry: dict) -> None:
    ...
    _ensure("features" in dsl and "features" in dsl["features"], "features.yaml missing features")
    ...
    _ensure("regimes" in dsl and "regimes" in dsl["regimes"], "regimes.yaml missing regimes")
    ...
    for regime in dsl["regimes"]["regimes"]:
        ...
        for rule in regime["rules"]:
            _ensure("id" in rule and rule["id"], f"regime rule missing id: {regime.get('id')}")
            ...
            _ensure("evidence" in rule, f"regime rule missing evidence: {regime.get('id')}")
            _validate_expression(rule["when"], f"regime {regime['id']}")

    _ensure("contexts" in dsl and "contexts" in dsl["contexts"], "contexts.yaml missing contexts")
    ...
    for context in dsl["contexts"]["contexts"]:
        ...
        _ensure("allowed_styles" in context, f"context missing allowed_styles: {context.get('id')}")
        _ensure("disallowed_styles" in context, f"context missing disallowed_styles: {context.get('id')}")
        _ensure("risk_budget" in context, f"context missing risk_budget: {context.get('id')}")
        for rule in context["rules"]:
            _ensure("id" in rule and rule["id"], f"context rule missing id: {context.get('id')}")
