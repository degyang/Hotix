...
def _resolve_index_csv_path(raw_dir: Path, index_id: str, config: dict[str, Any]) -> Path:
    candidates = []
    ...
    raise FileNotFoundError(f"Missing CSV for index '{index_id}'. Tried: {candidate_names}")


def load_all_data(raw_dir: Path | str, registry: dict[str, Any]) -> dict[str, pd.DataFrame]:
    raw_dir = Path(raw_dir)
    result = {}
    for index_id, config in registry["indices"].items():
        result[index_id] = load_csv_data(_resolve_index_csv_path(raw_dir, index_id, config))
    return result
