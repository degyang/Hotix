...
def evaluate_expression(expr: str, resolver, rule_id: str = ""):
    local_env = {
        ...
        "left": type("Obj", (), {"__getattr__": lambda _, name: resolver.resolve(f"left.{name}")})(),
        "right": type("Obj", (), {"__getattr__": lambda _, name: resolver.resolve(f"right.{name}")})(),
        "index": _CollectionNode("index", resolver),
        "pair": _CollectionNode("pair", resolver),
        "market": type("Obj", (), {"__getattr__": lambda _, name: resolver.resolve(f"market.{name}")})(),
        "true": True,
        "false": False,
    }
    return eval(normalize_expression(expr), local_env, {})
