...
from market_system.engine.models import PolicyOutput
...
def _apply_set(target: dict, values: dict) -> None:
    for path, value in values.items():
        if isinstance(value, dict) and path in target and isinstance(target[path], dict):
...
def score_policy(market, indices: dict, pairs: dict, policies_dsl: dict) -> dict:
    ...
    for _, rule in policies:
        if evaluate_expression(rule["when"], resolver):
            ...
            matched_rules.append({"rule_id": rule["id"], "priority": int(rule.get("priority", 0))})
