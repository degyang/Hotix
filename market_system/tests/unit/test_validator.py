            ...
            "regimes": [
                {
                    "id": "test",
                    "label": "Test",
                    "rules": [{"id": "r1", "when": "index.000300.trend_state == 'up'", "score": 1, "evidence": "ok"}],
                }
            ]
        },
        "contexts": {"contexts": []},
            ...
            "regimes": [
                {
                    "id": "test",
                    "label": "Test",
                    "rules": [{"id": "r1", "when": "index.000300.trend_state ==", "score": 1, "evidence": "bad"}],
                }
            ]
        },
        "contexts": {"contexts": []},
...
def test_validate_all_dsl_rejects_duplicate_feature_ids():
    registry = {"indices": {"000300": {"name": "沪深300"}}}
    dsl = {
        "features": {"features": [{"id": "dup", "type": "formula", "input": ["close"], "formula": "1", "output": "a"}, {"id": "dup", "type": "formula", "input": ["close"], "formula": "1", "output": "b"}]},
        "states": {"states": []},
        "patterns": {"patterns": []},
        "transitions": {"transitions": []},
        "salience": {"salience": {"scoring_rules": []}},
...
def test_validate_all_dsl_rejects_state_rule_missing_default():
    ...
    dsl = {
        "features": {"features": []},
        "states": {"states": [{"id": "trend_state", "output": "trend_state", "cases": []}]},
        "patterns": {"patterns": []},
        "transitions": {"transitions": []},
        "salience": {"salience": {"scoring_rules": []}},
        "pairs": {"pairs": []},
            ...
            "contexts": [
                {
                    "id": "cash",
                    "label": "Cash",
                    "rules": [{"id": "ctx_cash_01", "when": "len(market.relation_tags) == 0", "score": 2, "evidence": "缺乏明确结构主线"}],
                    "allowed_styles": ["观察"],
            ...
            "contexts": [
                {
                    "id": "cash",
                    "label": "Cash",
                    "rules": [{"id": "ctx_cash_01", "when": "market.market_regime.label ==", "score": 2, "evidence": "坏表达式"}],
                    "allowed_styles": ["观察"],
                    "disallowed_styles": ["主动进攻"],
                    "risk_budget": {"total_exposure": 0.1, "max_positions": 1, "max_single_name_weight": 0.05},
                }
            ]
        },
    }
