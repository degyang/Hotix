...
def test_score_policy_uses_cash_defaults_and_base_rule(
    ...
    result = score_policy(
        market_runtime_ready, ready_indices, ready_pairs, dsl_bundle["policies"]
    )

    assert result["setup_permissions"]["breakout"] == {
        "status": "forbidden",
        "size": "none",
    }
    assert result["setup_permissions"]["low_level_repair"] == {
        "status": "probe_only",
        "size": "tiny",
    }
...
def test_score_policy_applies_later_higher_priority_overrides(
    dsl_bundle, market_runtime_ready, ready_indices, ready_pairs
):
    market_runtime_ready.market_context = {
        "label": "Offense",
    ...
    assert result["setup_permissions"]["continuation_pullback"] == {
        "status": "allowed",
        "size": "normal",
    }
