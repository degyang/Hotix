...
from hotix.engine.loader import load_all_dsl, load_csv_data, load_registry
...
def test_project_root_contains_expected_entrypoints():
    ...
    assert (PACKAGE_ROOT / "config" / "index_registry.yaml").exists()
...
def test_load_csv_data_normalizes_dates_and_order():
    df = load_csv_data(FIXTURES_DIR / "000300.csv")
...
def test_load_all_data_prefers_symbol_named_csv_files(tmp_path):
    from hotix.engine.loader import load_all_data

    source = FIXTURES_DIR / "000300.csv"
