...
from hotix.engine.debug_report import (
    build_index_debug_report,
    build_market_debug_report,
    build_pair_debug_report,
)
...
from hotix.paths import PACKAGE_ROOT
...
def main() -> None:
    ...
    if target_date:
        ...
        if args.debug_index:
            print(
                json.dumps(
                    build_index_debug_report(payload, args.debug_index),
                    ensure_ascii=False,
                    indent=2,
                )
            )
        ...
        if args.debug_pair:
            print(
                json.dumps(
                    build_pair_debug_report(payload, args.debug_pair),
                    ensure_ascii=False,
                    indent=2,
                )
            )
        ...
        if args.debug_market:
            print(
                json.dumps(
                    build_market_debug_report(payload), ensure_ascii=False, indent=2
                )
            )
            return
        print(
            json.dumps(
                payload
                if args.dump_json
                else {"date": payload["date"], "market": payload["market"]},
                ensure_ascii=False,
                indent=2,
            )
        )
    ...
    if args.start or args.end:
        results = run_date_range(
            ctx, start=args.start, end=args.end, write_files=args.write_files
        )
        print(
            json.dumps({"dates_processed": len(results)}, ensure_ascii=False, indent=2)
        )
        return
